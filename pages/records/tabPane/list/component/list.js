...
export default function IncomeTypeList() {
  ...
  useEffect(() => {
    ...
    auth().onAuthStateChanged((user) => {
      ...
      if (user) {
        unsubscribe = db.collection(user.uid).doc('database').collection("income").onSnapshot(function (querySnapshot) {
  ...
  function showPromiseConfirm(record) {
    confirm({
      ...
      onOk() {
        auth().onAuthStateChanged((user) => {
          if (user) {
            db.collection(user.uid).doc('database').collection("income").doc(record.id).delete().then(function() {
  ...
  const handleReset = clearFilters => {
    ...
    const columns = [
      ...
      },
      {
        ...
        // width: '30%',
        ...getColumnSearchProps('incomeType'),
      },
      {
        key:'amount',
        title: 'Amount',
