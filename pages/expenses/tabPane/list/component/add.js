...
export default function AddExpensesType ({...props}){
    ...
    useEffect(() => {
      ...
      auth().onAuthStateChanged((user) => {
        if (user) {
          unsubscribe = db.collection(user.uid).doc('database').collection("expenses_type").onSnapshot(function (querySnapshot) {
    ...
    function onFinish (values) {
      ...
      console.log(values)
        try {
            auth().onAuthStateChanged((user) => {
                if (user) {
                    db.collection(user.uid).doc('database').collection('expenses').add(values)
