...
import { getArchivedFilesPerOffice, getUploadedFilesPerUser } from '../../../services/fecthData';
import { auth, db } from '../../../services/firebase';
const { TabPane } = Tabs;

const { Search } = Input;

export default function OfficeArchive({...props}) {
    ...
    const refresh = () => {
        auth().onAuthStateChanged((user) => {
            if(user){
                const data = getArchivedFilesPerOffice(props.office,decodeURI(campus))
                data.then(docs => {
                    setState({
                        initLoading: false,
                        list: docs,
